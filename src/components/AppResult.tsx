import { getMatchesBySquad, getSquadsByGroup, updatePointsGroup, updateResult } from "@/api/supabase";
...
const AppResult: React.FC<Props> = ({ item }) => {
    ...
    async function handleSubmitResult(event: React.FocusEvent<HTMLFormElement>) {
        ...
        console.log(score_home, score_away);
    ...
    async function calcPointsGroup() {
        ...
        const squadHome: SquadGroup[] = await getSquadsByGroup(squad_home.group, squad_home.id);
        const squadAway: SquadGroup[] = await getSquadsByGroup(squad_away.group, squad_away.id);
        ...
        matchesBySquadHome.map((match_squad_home: MatchDatum) => {
            if (match_squad_home.score_home != null && match_squad_home.score_away != null) {
                squadHome[0].goal_scored += match_squad_home.score_home;
                squadHome[0].goal_conceded += match_squad_home.score_away;
                squadHome[0].goal_difference += (squadHome[0].goal_scored - squadHome[0].goal_conceded);
        ...
        matchesBySquadAway.map((match_squad_away: MatchDatum) => {
            if (match_squad_away.score_home != null && match_squad_away.score_away != null) {
                squadAway[0].goal_scored += match_squad_away.score_away;
                squadAway[0].goal_conceded += match_squad_away.score_home;
                squadAway[0].goal_difference += (squadAway[0].goal_scored - squadAway[0].goal_conceded);
        ...
        await updatePointsGroup(squad_home.group, squadHome[0].id, squadHome[0].points, squadHome[0].goal_scored, squadHome[0].goal_conceded, squadHome[0].goal_difference);

        await updatePointsGroup(squad_home.group, squadAway[0].id, squadAway[0].points, squadAway[0].goal_scored, squadAway[0].goal_conceded, squadAway[0].goal_difference);
        ...
        <><form action="" onSubmit={handleSubmitResult}>
            ...
            <span>{squad_home.name}</span>
            {
                isActive ? (
                    ...
                    score_home != null || score_away != null ? (`${score_home} - ${score_away}`) : (<p>Risultato da inserire</p>)
                )
            }
            ...
            {
                !isActive ? (<button type="button" onClick={toggleDisabled}>{(score_home != null || score_away != null) ? 'Modifica' : 'Inserisci'}</button>) : (null)
            }
